...
params.fastqDir="/home/nova/Desktop/alen-belen/results_reads_filtered/samtools_extract"
params.genome="/home/nova/Desktop/alen-belen/results/bakta"
params.GENOMAD_BAKTA_PATH = "/home/nova/Desktop/alen-belen/results/genomad_bakta"
params.MINIMAP2_PATH = "/home/nova/Desktop/alen-belen/results/minimap2_bakta"
params.OUTPUT_PATH = "/home/nova/Desktop/alen-belen/results/mosdepth_mge_genes/virus"
params.LOG_PATH = "/home/nova/Desktop/alen-belen/scripts/mge/mosdepth_logs"
...
process GenerateSampleList {
    ...
    for barcode_dir in "\${GENOMAD_BAKTA_PATH}"/*; do
        ...
        gff3_file="\${GENOMAD_BAKTA_PATH}/\${barcode}/virus/virus_genes_bakta.gff3"
...
process GenerateBED {
    ...
    output:
    tuple val(barcode), path("${barcode}_virus_genes_bakta.bed"), path(gff3_file), path(bam_file), path(bai_file)
    
    script:
    """
    echo "Generating BED file for ${barcode}"
    grep -P -v '\tregion\t' ${gff3_file} | awk '{split(\$9, a, ";"); split(a[1], b, "="); print \$1"\t"\$4"\t"\$5"\t"b[2]}' > ${barcode}_virus_genes_bakta.bed   
    """
}
...
process CombineDepthGFF3 {
    ...
    tuple val(barcode), path(bed_file), path(gff3_file), path(bam_file), path(bai_file)
    output:
    path("${barcode}_virus_genes_bakta_mosdepth.gff3")
    path("${barcode}_mosdepth.regions.bed")
    path(bed_file)
    
    publishDir "${params.GENOMAD_BAKTA_PATH}/${barcode}/virus", pattern: "*.gff3", mode:'copy'
    publishDir "${params.OUTPUT_PATH}/${barcode}", pattern: "*regions*", mode: 'copy'
    publishDir "${params.GENOMAD_BAKTA_PATH}/${barcode}/virus", pattern: "*bakta.bed", mode: 'copy'

    script:
    regions_bed = "${barcode}_mosdepth.regions.bed"
    output_gff3 = "${barcode}_virus_genes_bakta_mosdepth.gff3"
    """
