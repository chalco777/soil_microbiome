...
params.OUTPUT_PATH = "/home/nova/Desktop/alen-belen/results/mosdepth_mge_genes/plasmid"
...
process GenerateBED {
    ...
    output:
    tuple val(barcode), path("${barcode}_plasmid_genes_bakta.bed"), path(gff3_file), path(bam_file), path(bai_file)
    
    script:
    """
    echo "Generating BED file for ${barcode}"
    grep -P -v '\tregion\t' ${gff3_file} | awk '{split(\$9, a, ";"); split(a[1], b, "="); print \$1"\t"\$4"\t"\$5"\t"b[2]}' > ${barcode}_plasmid_genes_bakta.bed   
    """
}
...
process CombineDepthGFF3 {
    ...
    tuple val(barcode), path(bed_file), path(gff3_file), path(bam_file), path(bai_file)
    output:
    path("${barcode}_plasmid_genes_bakta_mosdepth.gff3")
    path("${barcode}_mosdepth.regions.bed")
    path(bed_file)
    
    publishDir "${params.GENOMAD_BAKTA_PATH}/${barcode}/plasmid", pattern: "*.gff3", mode:'copy'
    publishDir "${params.OUTPUT_PATH}/${barcode}", pattern: "*regions*", mode: 'copy'
    publishDir "${params.GENOMAD_BAKTA_PATH}/${barcode}/plasmid", pattern: "*bakta.bed", mode: 'copy'

    script:
    regions_bed = "${barcode}_mosdepth.regions.bed"
    output_gff3 = "${barcode}_plasmid_genes_bakta_mosdepth.gff3"
    """
